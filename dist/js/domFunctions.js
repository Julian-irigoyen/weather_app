...
const displaySixDayForecast = (weatherJson) => {
  for (let i = 1; i <= 6; i++) {
    const dfArray = createDailiyForecastDivs(weatherJson.daily[i]);
    displayDailyForecast(dfArray);
  }
};

const createDailiyForecastDivs = (dayWeather) => {
  ...
  const dayIcon = createDailiyForecastIcon(
...
const createDailiyForecastIcon = (icon, altText) => {
